...
void solve()
{
    ...
    vector <vector<ll>> vp(5002, vi(5002, 0));

    // vp[i][j] -> no. of sequences to make sum i using first j elemnts,
    // second stores max ai over these
    ...
    FORL(i, 1, 5002)
    {
        FOR(j, 5002)
        {
            ...
            if(i - b[j - 1] >= 0)
                vp[i][j] = (vp[i - b[j-1]][j-1] + vp[i][j-1]) % MOD;
            else
                vp[i][j] = vp[i][j-1];
        }
    }
