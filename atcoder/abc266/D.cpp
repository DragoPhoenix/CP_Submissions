...
void solve()
{
	...
	vi time(n), idx(n), sz(n);
	FOR(i, n)
		cin >> time[i] >> idx[i] >> sz[i];
	...
	FORL(i, 1, N)
	{
		...
		{
			vi poss = pos;
			pos[4] = max(pos[4], poss[3]);
			FORR(j, 3, 1)
			{
				pos[j] = max(pos[j], poss[j+1]);
				pos[j] = max(pos[j], poss[j-1]);
			}
			pos[0] = max(pos[0], poss[1]);
			
			x = idx[k];
			pos[x] = max(pos[x], poss[x] + sz[k]);
			if(x+1 < 5)
				pos[x] = max(pos[x], poss[x+1] + sz[k]);
			if(x-1 >= 0)
				pos[x] = max(pos[x], poss[x-1] + sz[k]);
			k++;
			if(k == n)
