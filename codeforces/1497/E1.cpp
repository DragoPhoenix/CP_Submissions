...
const ll N=1e7+5;
vi classify_numbers(N,0);

void pre(ll n=N) 
{
    FORL(i,1,n+1)classify_numbers[i]=i;
    FORL(i,2,n+1)
    {
        ll x=sqrt(i);
        if(x*x==i)
        {
            ...
            while(y * i < n + 1)
            {
                ll j=classify_numbers[y*i];
                while(j%i==0)
                    j/=i;
                classify_numbers[y*i]=j;
                y++;
            }
            classify_numbers[i]=1;
        }
    }
} 
...
void solve()
{
    ...
    FOR(i,n)
    {
        if(st.find(classify_numbers[a[i]])!=st.end())
        {
            st.clear();
            st.insert(classify_numbers[a[i]]);
            z++;
        }
        else
            st.insert(classify_numbers[a[i]]);
