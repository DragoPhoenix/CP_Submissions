...
void solve()
{
    ...
    cin >> n >> c;
    vi a(n), b(n-1), reach(n, 0), only_fans(n, 0), with(n ,0);
    FOR(i, n) cin >> a[i];
    FOR(i, n-1) cin >> b[i];

    FOR(i, 1)
    {
        only_fans[i] = ceil((double)c / a[i]);
    }
    ...
    FORL(i, 1, n)
    {
        a1 = ceil(((double)b[i-1] - with[i-1])/ a[i-1]);
        with[i] = a1 * a[i-1] - b[i-1] + with[i-1];
        reach[i] = reach[i-1] + a1 + 1;
        only_fans[i] = ceil(((double)c - with[i] ) / a[i]);
    }
