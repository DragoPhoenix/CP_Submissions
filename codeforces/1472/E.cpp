...
void solve()
{
	...
	vector <pair<ll,ll> > bm(n);
	vector <pair<ll,ll> > cm(n);
	...
	FORL(i,1,n)
	{
		// bm[i]=min(bm[i-1],a[b[i].S].S);
		if(a[b[i].S].S<bm[i-1].F)
			bm[i]={a[b[i].S].S,i};
		else
			bm[i]=bm[i-1];
		// cm[i]=min(a[c[i].S].F,cm[i-1]);
		if(a[c[i].S].F<cm[i-1].F)
			cm[i]={a[c[i].S].F,i};
		else
			cm[i]=cm[i-1];
	}
	FOR(i,n)
	{
		...
		it=lower_bound(c.begin(), c.end(),p)-c.begin();
		it--;
		// pr1(it)
		if(it>=0)
		{
