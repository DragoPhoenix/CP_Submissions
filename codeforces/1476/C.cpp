...
void solve()
{
    ...
    FORL(i,1,n)
    {
        vp[i]=c[i]-1+2+abs(b[i]-a[i]); 
        // if(vp[i-1])
        // pr3(max3(0,vp[i-1],vp1[i-1]),(c[i-1]-1-abs(a[i]-b[i])),c[i]-1+2)
        vp1[i]=c[i]-1+2+(max3(0,vp[i-1],vp1[i-1])-abs(a[i]-b[i]));
        if(max3(0,vp[i-1],vp1[i-1])==0)
            vp1[i]=0;
        if(a[i]==b[i])
            vp1[i]=vp[i];
        // if((c[i-1]-1-abs(a[i]-b[i]))==0)
        // {
        //     vp1[i]=c[i]-1+2+(max3(0,vp[i-1],vp1[i-1]))-c[i-1]+1;
        // }
    }
    // debugv(vp)
    // debugv(vp1)
