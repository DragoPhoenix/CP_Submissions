...
void solve()
{
    ...
    FORL(i, 1, points.size())
    {
        z = points[i].F - x + points[i].S - y;
        dist.PB(z);
        x = points[i].F;
        y = points[i].S;
    }
    ...
    for(auto e : points)
    {
        if(abs(e.F - xs) + abs(e.S - ys) < z)
        {
            z = abs(e.F - xs) + abs(e.S - ys);
            a1 = k;
        }
        k++;
    }
    ...
    vi prdis(dist.size() + 1, 0);
    
    FORL(i, 1, dist.size() + 1)
        prdis[i] += dist[i-1] + prdis[i-1];
    ...
    FOR(i, n)
    {
        FORL(j, i, n)
        {
            a1 = abs(xs - points[i].F) + abs(ys - points[i].S);
            a2 = abs(xs - points[j].F) + abs(ys - points[j].S);
            if(min(a1, a2) + prdis[j] - prdis[i] <= t)
                z = max(z, j - i + 1);   
        }
    }
