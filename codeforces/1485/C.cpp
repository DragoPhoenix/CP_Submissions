...
void solve()
{
    ...
    cout << z << "\n";
    /*
    [a1 + 1, y + 1] -> how many numbers in this range * i <= x
    a = b * x + y
    x, y >= 0
    for x = y:
    a = b * x + x ; x < b
    a = v * (b + 1) ; 1 <= v < b
    => b + 1 is a factor of a
    ...
    From y side:
    For 1: divisible by 2 => all even numbers  -> INVALID as mod equals 0 always
    For 2: div by 3 => x / 3
    
    For 2: b = 2 => * can be max 1 => b - 1 values
    For 3: 2 values max: a = 4, 8
    For 4: 3 values max: a = 5, 10, 15
    
    can take max values for all i such that (i - 1) * (i + 1) <= x
    let this max value is a1 => z += a1 * (a1 - 1) / 2
    
    now for a1 + 1 onwards, we need count of multiples <= x
